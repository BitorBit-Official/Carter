    ...
    public class ArcadeVehicleController : NetworkBehaviour
    {
        ...
        private void Update()
        {
            if(IsOwner)
            {
                ...
                if(verticalInput > 0)
                {
                    CM_cam.GetCinemachineComponent<CinemachineBasicMultiChannelPerlin>().m_AmplitudeGain = 1f;
                }
                else
                {
                    CM_cam.GetCinemachineComponent<CinemachineBasicMultiChannelPerlin>().m_AmplitudeGain = 0.3f;
                }
                Visuals();
            }
            AudioManager();
